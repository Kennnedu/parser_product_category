class CategoryExtractor
  ...
  def initialize(category_url, filename)
  ...
  def self.call(category_url, filename)
    ...
    new(category_url, filename = 'extracted_data').call
  end
  ...
  def extract_multiproducts
    CSV.open("#{@filename}.csv", 'wb') do |csv|
      ...
      get_links_multiproducts.each do |links_multiproduct|
